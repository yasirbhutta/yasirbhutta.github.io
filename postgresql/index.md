---
layout: page
title: PostgreSQL Home
description: Beginner-friendly notes on PostgreSQL stored procedures with practical dvdrental examples.
keywords: PostgreSQL stored procedures, dvdrental database, PL/pgSQL, SQL procedures, CALL statement, database programming
toc: toc/postgresql.html
topic: postgresql
breadcrumb: 
- title: PostgreSQL
  url: /postgresql/
---

## Contents

- [PostgreSQL Sample Database](docs/dvdrental.md)
- [Fix 'psql' is not recognized on Windows](docs/fix-psql.md)
- [Stored Procedures in PostgreSQL (Using dvdrental)](docs/stored-procedures.md)
  - [Advantages of Using Stored Procedures in PostgreSQL](docs/stored-procedures.md)
- [Introduction to Triggers in PostgreSQL (Using dvdrental Database)](docs/triggers.md)

### [Database Management](docs/databases/index.md)

- [Create a Database](docs/databases/create-database.md)
- [Rename a Database](docs/databases/rename-database.md)
- [Drop a Database](docs/databases/drop-database.md)

### Roles and Privileges

- [Create Roles](docs/roles-privileges/create-roles.md)
- [Grand Privileges](docs/roles-privileges/grant-privileges.md)
- [Revoke Privileges](docs/roles-privileges/revoke-privileges.md)
- [Drop Roles](docs/roles-privileges/drop-roles.md)
- [🧪 Lab Exercise: Managing Roles in PostgreSQL](docs/roles-privileges/lab-exercise1-roles-privileges.md)

## Assignments
  
- [PostgreSQL Python](https://neon.com/postgresql/postgresql-python)
- [Final Project Guidelines for Database](projects/final-dba-project-guidelines.md)
  
## Presentation Topics

## 

- [Review Questions](docs/review-questions.md)


## See Also

- [PostgreSQL Tutorial](https://neon.com/postgresql/tutorial)